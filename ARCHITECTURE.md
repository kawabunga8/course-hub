# RCS Suite — Multi-Year Data Architecture

> This document lives in Course Hub because Course Hub owns the data it
> describes. It was written in the CourseBoard repo, which reads this data and
> never writes it, so the design for the schema sat in a consumer rather than in
> its owner. The content below is unchanged apart from three corrections, each
> marked **Correction**.
>
> Course Hub is the source: every table listed in `CLAUDE.md` under "owned here"
> is written here and read elsewhere.
...
> **Correction — who actually writes what.** Measured by which app issues
> inserts, updates or deletes:
>
> | Table | Written by |
> | --- | --- |
> | `students` | Course Hub, TOC-Dayplans, Report Card Tool |
...
> plans, is what makes the rule true for that table.
...
> **Correction.** There is a fifth: `public.student_marks.class_id`, defined in
> `supabase/shared-schema.sql`. It is what the single call site powers - the
> optional "Class" tag on a student mark, and the label on saved marks. Step 6
> of the decommissioning sequence, "drop `public.classes`", fails on this FK
> until marks are retagged, so step 2 is a small data migration rather than a
> UI change.
...
> **Correction.** This and the note under step 3 cannot both be current. That
> note records the 2026-27 import as done - 266 enrollments, 218 students, 76
> returners matched to existing rows. If a second year is already in, step 5's
> cheap window has closed and it is the reconciliation project this paragraph
> warns about. Check before relying on either sentence. Splitting one student into per-year rows has not happened yet, so
